...
// certToPEM() and getSigningKey() from suggestion in https://knowledge.udacity.com/questions/203918
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
  ...
  // Based on method here: https://auth0.com/blog/navigating-rs256-and-jwks/
  // to get the decoded jwt.header.kid, NOT authHeader.kid
  // from the article above kid: is the unique identifier for the key
  const jwt: Jwt = decode(token, { complete: true }) as Jwt

  let signedKey = await getSigningKey(jwksUrl, jwt.header.kid)

  // verify is imported from jsonwebtoken
